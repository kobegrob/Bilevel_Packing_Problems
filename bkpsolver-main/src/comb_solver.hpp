...
private:
    ...
    struct __attribute__((packed)) LowerTableEntry {
        int wt;
        P pft;
        bool operator==(const LowerTableEntry& other) const {
    ...
    P &d_tab_val(size_t i, int up_cap, int lo_cap);
    P s_tab_val(size_t i, int up_cap, int lo_cap);
...
BKPSolution<P> Comb_BKPSolver<P>::solve() {
    ...
    if (!lb_only) {
        ...
        auto initial_bound_end_time = Clock::now();
        std::cout << "initial_bound_test_time "
                  << duration<double, std::milli>(initial_bound_end_time - init_time).count()
                  << std::endl;
    ...
    auto lb_end_time = Clock::now();
    std::cout << "dp_lb_time "
              << duration<double, std::milli>(lb_end_time - lb_start_time).count()
              << std::endl;
    ...
    pre_is_sparse.push_back(prefix_density_threshold > 0);
    d_pre_tab_idx = 0;
    ...
    auto bnb_end_time = Clock::now();
    std::cout << "bnb_time "
              << duration<double, std::milli>(bnb_end_time - lb_end_time).count()
              << std::endl;
    std::cout << "opt_time "
              << duration<double, std::milli>(opt_time - lb_end_time).count()
              << std::endl;
    std::cout << "proof_time "
              << duration<double, std::milli>(bnb_end_time - opt_time).count()
              << std::endl;
    std::cout << "total_time "
              << duration<double, std::milli>(bnb_end_time - init_time).count()
              << std::endl;
...
void Comb_BKPSolver<P>::solve_rec(size_t i, int up_cap, int lo_cap, int pft) {
    max_depth = std::max(max_depth, static_cast<uint64_t>(i));
    sum_depth += i;
